...
% make the otherMap the size of fixationMap
if size(otherMap, 1)~=size(fixationMap, 1) || size(otherMap, 2)~=size(fixationMap, 2)
    otherMap = imresize(otherMap, size(fixationMap));
end
